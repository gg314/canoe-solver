    ...
    while keep_running:
      if len(self.exp_buffer) - self.idx < batch_size:
        ...
        if len(self.filenames) == 0:
          ...
          with h5py.File("./generated_experience/" + current_file + ".h5") as exp_file:
            if len(self.exp_buffer):
              self.exp_buffer = combine_experience([self.exp_buffer, load_experience(exp_file)])
            else:
...
class ExperienceBuffer:
  ...
  def serialize(self, h5file):
    ...
    h5file['experience'].create_dataset('advantages', data=self.advantages, dtype='float32')
