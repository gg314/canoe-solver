...
class GreedyAgent(Agent):
    ...
    def find_winning_move(self, game) -> Optional[Move]:
...
class DeepLearningAgent(Agent):
    """This agent attempts to use Dense (fully-connected) layers to replicate a policy"""
...
class ACAgent(Agent):
    ...
    def train(self, experience, learning_rate, batch_size=128):
        """Train the agent's model with keras"""
        # sgd = tensorflow.keras.optimizers.SGD(learning_rate=learning_rate, clipvalue=.2)
        # adadelta = tensorflow.keras.optimizers.Adadelta(learning_rate=learning_rate, clipvalue=.2)
        adam = tensorflow.keras.optimizers.Adam(
            learning_rate=learning_rate, beta_1=0.9, beta_2=0.999, clipvalue=0.1
        )  # 000000007 = .553 # 0000007 adj weights
        self.model.compile(
    ...
    def generator_train(self, data_generator, learning_rate):
        """Train the agent's model with keras, loading data with a generator"""
        # sgd = tensorflow.keras.optimizers.SGD(learning_rate=learning_rate, clipvalue=.2)
        # adadelta = tensorflow.keras.optimizers.Adadelta(learning_rate=learning_rate, clipvalue=.2)
        adam = tensorflow.keras.optimizers.Adam(
            learning_rate=learning_rate, beta_1=0.9, beta_2=0.999, clipvalue=0.00000001
        )  # base .00000007
        self.model.compile(
            loss=["categorical_crossentropy", "mse"],
            loss_weights=[1.0, 1.0],
