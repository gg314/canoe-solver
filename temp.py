...
from canoebot.experience import (
    ExperienceBuffer,
    combine_experience,
    load_experience,
    DataGenerator,
...
def split_data():
    ...
    for exp_filename in experience_files:
        exp_buffer = load_experience(
            h5py.File("./generated_experience/" + exp_filename + ".h5")
        )
    ...
    long = ExperienceBuffer(
        exp.states[0:keep],
        exp.actions[0:keep],
        exp.rewards[0:keep],
        exp.advantages[0:keep],
    )
    short = ExperienceBuffer(
        exp.states[keep:], exp.actions[keep:], exp.rewards[keep:], exp.advantages[keep:]
    )
...
def make_progress_bar(wins, trials, total, winner):
    ...
    print(
        f"{trials + 1:3}/{total:3} |{pre_bars}|{post_bars}| ({(100 * wins / (trials + 1)):.1f}%)"
    )
            ...
            for i in range(6):
                for j in range(13):
                    if (13 * i + j) not in zzzzzz:
                        _text = ax[0].text(
                            j, i, 13 * i + j, ha="center", va="center", color="w"
                        )
            fig.tight_layout()
            plt.show()
...
def make_heatmap(arr, title):
    ...
    ax.set_axis_off()
    ax.set_title(
        title + "\n", fontsize=16, color="#ce9178", fontfamily="DINPro"
    )  # d4d4d4
    # for i in range(6):
    #     for j in range(13):
    #         if (13*i+j) not in zzzzzz:
